...
@JvmOverloads constructor(private val tag: String = DEFAULT_TAG, messagePrefix: String? = null) {
    ...
    fun v(t: Throwable, format: String, vararg args: Any) {
        if (isLoggable(Log.VERBOSE)) Log.v(tag, toMessage(format, *args), t)
    }

    fun d(format: String, vararg args: Any) {
        if (isLoggable(Log.DEBUG)) Log.d(tag, toMessage(format, *args))
    }

    fun d(t: Throwable, format: String, vararg args: Any) {
        if (isLoggable(Log.DEBUG)) Log.d(tag, toMessage(format, *args), t)
    }

    fun i(format: String, vararg args: Any) {
        if (isLoggable(Log.INFO)) Log.i(tag, toMessage(format, *args))
    }

    fun i(t: Throwable, format: String, vararg args: Any) {
        if (isLoggable(Log.INFO)) Log.i(tag, toMessage(format, *args), t)
    }

    fun w(format: String, vararg args: Any) {
        if (isLoggable(Log.WARN)) Log.w(tag, toMessage(format, *args))
    }

    fun w(t: Throwable, format: String, vararg args: Any) {
        if (isLoggable(Log.WARN)) Log.w(tag, toMessage(format, *args), t)
    }

    fun e(format: String, vararg args: Any) {
        if (isLoggable(Log.ERROR)) Log.e(tag, toMessage(format, *args))
    }

    fun e(t: Throwable, format: String, vararg args: Any) {
        if (isLoggable(Log.ERROR)) Log.e(tag, toMessage(format, *args), t)
    }
